...
class ShadcnProfileScreen extends StatefulWidget {
  const ShadcnProfileScreen({Key? key}) : super(key: key);
...
class _ShadcnProfileScreenState extends State<ShadcnProfileScreen>
  ...
  int _points = 0;
  int _level = 1;
  int _streak = 7; // Days active
  int _rank = 42; // Leaderboard position
  ...
  Future<void> _loadUserStats() async {
    try {
      ...
      // Calculate points (10 per report, 50 bonus per resolved)
      final calculatedPoints = (reported * 10) + (resolved * 50);
      final calculatedLevel = (calculatedPoints / 500).floor() + 1;

      if (mounted) {
        setState(() {
          _issuesReported = reported;
          _issuesResolved = resolved;
          _points = calculatedPoints;
    ...
    return Scaffold(
      ...
      body: Consumer<AuthProvider>(
        builder: (context, authProvider, child) {
          ...
          final city = user.userMetadata?['city'] ?? 'Chennai';
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildImpactItem(theme, '${_issuesResolved * 5}', 'Trees Saved',
                  Icons.park_outlined),
              _buildImpactItem(theme, '${_issuesReported * 10}',
                  'People Helped', Icons.people_outline),
              _buildImpactItem(theme, '${_issuesResolved * 2}kg', 'CO₂ Reduced',
  ...
  Widget _buildImpactItem(
      ShadThemeData theme, String value, String label, IconData icon) {
    return Column(
      children: [
        Icon(
          icon,
